...
const app = Vue.createApp({
    setup(context) {
        ...
        const finished = async (msg) => {
            ...
            const request = {
                SurveyID: (surveyNum.value || 0), 
                TicketID: (ticketNum.value || 0),
                Certificate: chooseCertificate.value,
                Result: msg.Result || msg.score,
                Desglose: msg.Desglose || JSON.stringify(msg.points)
            }

            
            //Request
            const res = await axios.post( api+'/SaveData', request )
        ...
        const surveyCheck = async () => {
            ...
            if(res.data.d==0){
                ...
                dialogErrorTxt.value = 'Hemos registrado tu participación del día de hoy. <br>Gracias por tu opinión.'
            }
        }
        const ticketCheck = () => {
